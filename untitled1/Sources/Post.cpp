...
void PostCreate::handleRequest(const Wt::Http::Request& request, Wt::Http::Response& response)
{
    ...
    PreparedStatement_setString(p, 1, hR.objectRequest["date"].toString().toStdString().c_str());
    PreparedStatement_setInt(p, 2, hR.objectRequest["thread"].toInt());
    PreparedStatement_setString(p, 3, hR.objectRequest["message"].toString().toStdString().c_str());
    PreparedStatement_setString(p, 4, hR.objectRequest["user"].toString().toStdString().c_str());

    PreparedStatement_setString(p, 5, hR.objectRequest["forum"].toString().toStdString().c_str());
    ...
    if (hR.objectRequest["parent"] != QJsonValue::Null) {
        ...
        PreparedStatement_setString(p, 6, "");
    }
    ...
    PreparedStatement_setString(p, 12, path.toStdString().c_str());
    ...
    hR.objectResponce["response"] = PostInfo::getFullPostInfo(Connection_lastRowId(con), isThreadExist);
